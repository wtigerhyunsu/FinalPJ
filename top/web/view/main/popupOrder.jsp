...
<script>
	...
	function plusItem(pi, itemIndex) {
		...
		document
				.querySelector("body > div > div > div > div > div > div.row > div > div.col-12.panel-footer.basket-footer > div > div > div").innerHTML = "총합: "
				+ totalPrice.toString();
		;
	}
	//물건 수량을 빼는 함수 
	...
	function destroyItem(di, itemIndex) {
		...
		if (totalPrice > 0) {
			totalPrice = totalPrice - SelectedItemMoney[itemIndex]
					* (SelectedItemArray[di - 1] - 1);// di와 di-1로 구분 되는 것은 첫번 째 SelectedItemMoney는 선택이 있고 두번째 SelectedItemArray는 선택이 없기 때문이다.
		} else {
		}
	...
	function myIngredientFunction(totalAddedItemIndex) {//n번째 항목과 n번째 아이템
		...
		var DivOfmyIngredient = "div." + z.substring(0, 3) + '.'
				+ z.substring(4);
		console.log("itemList 는" + itemList);//itemList 중 받아온 n번 째를 표시해야 한다. 
		var myTmp = "#itemDivId" + totalAddedItemIndex
				+ "> select option:selected";
		var indexOfItemList = $(myTmp).index();//아이템 배열의 인덱스 
		console.log("n번 째의 값은 "
				+ DivOfmyIngredient.charAt(DivOfmyIngredient.length - 1));
		...
		myNewCode += itemList[indexOfItemList].IngName
				+ "<button onclick=\"plusItem(" + totalAddedItemIndex + ","
				+ indexOfItemList + ")\">+</button>";
		myNewCode += "<button onclick=\"minusItem(" + totalAddedItemIndex + ","
				+ indexOfItemList + ")\">-</button>";
		//myNewCode += "<button onclick=\"minusItem(" + totalAddedItemIndex + ")\">-</button>";
		myNewCode += "<button onclick= \"destroyItem(" + totalAddedItemIndex
				+ "," + indexOfItemList + ")\">물품삭제</button></div>";//아이템 지우기 
		myNewCode += "<div class=\"col-2 align-right\">";
		//여기서 버튼을 눌러서 수량을 증감한다. 
		myNewCode += "<span class=\"sub\">" + "1" + "</span>"
				+ itemList[indexOfItemList].IngUnit;
		myNewCode += "</div>";
		myNewCode += "<div class=\"col-3 align-right\">";
		myNewCode += "<span class=\"sub\">" + "$" + "</span>"
				+ itemList[indexOfItemList].IngPrice;
		myNewCode += "</div>";
		myNewCode += "</div>";
		DivOfmyIngredient = DivOfmyIngredient.substring(0,
				DivOfmyIngredient.length - 1)
				+ totalAddedItemIndex;
	...
	function getData() {
		$.ajax({
			...
			success : function(data) {
				//여기서 데이터를 받아온다
				console.log("data는 " + data + "data type은 " + typeof (data)
						+ " data length 는" + data.length);
				console.log(data);
				itemList = data;
				SelectedItemMoney = [];
		...
		$
				.ajax({
					...
					success : function(data) {
						...
						$('#template > div.col-12.panel-body.basket-body')
								.append(myTable);
						var myTemp;
						myTemp += "";
						for (var i = 0; i < data.length; i++) {
	...
	$(document).ready(function() {
		...
		var chainOrHq = 0;
		if (loginId.includes("cafe_TOP_hq")) {// 체인의 가맹점용
			chainOrHq = 0;
...
<style>
#rowC { //가로 스크롤 white-space:nowrap; /* 가로스크롤시 중요한 속성 */
	overflow-x: auto;
	overflow-y: hidden;
						...
						<div class="row">
							<div class="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
								<div id="template" class="row panel-wrapper">
									<div class="col-12 panel-header basket-header"
										...
										<div class="row">
											...
											<div class="col-6 order-number align-right">
												<span class="description"> </span><br> <span
													class="emphasized"> </span>
											</div>
										</div>
										<div class="row"
									...
									<div class="col-12 panel-body basket-body""></div>
								</div>
								<div class="col-12 panel-footer basket-footer">
									...
									<div class="row">
										<div class="col-8 align-right description">
											...
											<input type="button" onclick="addItemToContainer()"
												value="Add" style="color: red; float: left;" />
